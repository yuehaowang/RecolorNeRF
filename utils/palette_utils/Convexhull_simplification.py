...
def write_convexhull_into_obj_strs(hull):
    ret = []
    ...
    for index in range(hvertices.shape[0]):
        ret.append('v '+str(hvertices[index][0])+' '+str(hvertices[index][1])+' '+str(hvertices[index][2])+'\n')
    for index in range(hfaces.shape[0]):
        ret.append('f '+str(hfaces[index][0])+' '+str(hfaces[index][1])+' '+str(hfaces[index][2])+'\n')
    return ret
